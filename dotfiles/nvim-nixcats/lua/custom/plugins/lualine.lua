return {
  {
    ...
    lazy = false, -- încarcă imediat la start
    config = function()
      -- culori
      local colors = {
        bg       = '#1F1F28',
      ...
      -- secțiune stânga
      ins_left({ "b:gitsigns_head", icon = "", color = { fg = colors.violet } })
      ...
      ins_left({ "diff", source = diff_source, colored = true, symbols = { added = " ", modified = "󰝤 ", removed = " " },
      ...
      function custom_filetype:update_status()
        ...
        return highlight.component_format_highlight(vim.bo.modified and self.status_colors.modified or self.status_colors.saved) .. data
      end
